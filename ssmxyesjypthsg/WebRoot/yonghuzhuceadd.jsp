...
function hsgcheck() {
		...
		if(yonghuming==""||(yonghuming.length<6||yonghuming.length>12)){
			 $("#clabelyonghuming").html("<font color=red>学号不能为空且长度在3～12位之间！</font>");
		...
		else
		{
			...
			$.ajax({
				...
				if(result.info=="ng"){
					$("#clabelyonghuming").html("<font color=red>学号已存在，请更换！</font>");
					$("input[id=yonghuming]").focus();
					$("#querenzhuce").attr("disabled","disabled");
					return false;
				}
...
function checkform()
{
	var yonghumingobj = document.getElementById("yonghuming"); if(yonghumingobj.value==""){document.getElementById("clabelyonghuming").innerHTML="&nbsp;&nbsp;<font color=red>请输入学号</font>";return false;}else{document.getElementById("clabelyonghuming").innerHTML="  "; } 
...
<form action="addYonghuzhuce.do" name="form1" method="post">
		...
		<tr ><td width="200">邮箱：</td>
		  <td><input name='youxiang' type='text' id='youxiang' value='' onBlur='checkform()' style='border:solid 1px #000000; color:#666666' />
		    &nbsp;
		    <label id='clabelyouxiang' />必需邮箱格式</td></tr>
		...
		<tr 1><td width="200">学院部门：</td><td><select name='dizhi' id='dizhi'>
